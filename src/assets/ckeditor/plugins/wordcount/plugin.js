...
CKEDITOR.plugins.add("wordcount",
    {
        ...
        init: function(editor) {
            ...
            var defaultConfig = {
                ...
                charCountGreaterThanMaxLengthOnPasteEvent(currentLength, maxLength) {
                    dispatchEvent('charCountGreaterThanMaxLengthOnPasteEvent', currentLength, maxLength);
                }
            };
                    ...
                    if (!config.warnOnLimitOnly && (config.maxWordCount > 0 || config.maxCharCount > 0 || config.maxParagraphs > 0)) {
                        ...
                        if (config.maxCharCount > 0 && charCount > config.maxCharCount && config.hardLimit) {
                            if (!notification.isVisible()) {
                                config.charCountGreaterThanMaxLengthOnPasteEvent(charCount, config.maxCharCount);
                                if (config.showPasteWarningNotification) {
                                    notification.show();
                                }
                            }
                            event.cancel();
